-- Stack32 — the free grant becomes 6 credits instead of 10.
--
-- Same one-time, never-renewing shape as before; only the size changes. The
-- rate stays exactly the Pro rate ($0.055/credit), so 6 credits carry
-- $0.33 of AI budget and a balance inherited from a lapsed subscription still
...
begin
  if p_plan_key = 'free' then
    return 0.33 * greatest(p_credits_monthly, 6) / 6.0;
  end if;
